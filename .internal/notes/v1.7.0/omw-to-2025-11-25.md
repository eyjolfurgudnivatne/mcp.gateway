...
**Estimated effort:** 3-4 uker (60-80 timer)
...
- Uses regex: `^[a-zA-Z0-9_-]{1,128}$` (no dot support!)
- **Action:** Update `ToolMethodNameValidator` to allow dot (`.`)
...
#### 2. Icon Support
**New in 2025-11-25:**
- Tools, Prompts, Resources can now have `icons` array
- Format:
  ```json
  "icons": [
    {
...
**Action:** Add optional `icons` field to `McpToolAttribute`, `McpPromptAttribute`, `McpResourceAttribute`
...
2. Update tool name validator to allow dots (`.`)
3. Add `icons` support to attributes
4. Add `structuredContent` and `outputSchema` to tools
5. Distinguish tool execution errors from protocol errors
6. Run all tests, ensure backward compatibility
...
**Estimated effort:** 1 uke (40 timer)
...
| Phase | Duration | Effort (timer) | Release | Deliverables |
|-------|----------|----------------|---------|--------------|
| **Phase 0** | 1 uke | 40 timer | v1.6.5 | Minor 2025-11-25 updates |
| **Phase 1** | 2-3 uker | 40-60 timer | v1.7.0 | Streamable HTTP transport |
| **Phase 2** | 1 uke | 20 timer | v1.7.5 | SSE-based notifications |
| **Total** | **4-5 uker** | **100-120 timer** | v1.7.5 | Full 2025-11-25 compliance |
